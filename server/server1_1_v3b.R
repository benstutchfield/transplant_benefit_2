...
betas_current = drop_na(betas_surv %>% select(1:12))
surv_cancer_current = drop_na(betas_surv %>% select(13:14))
surv_noncancer_current =  drop_na(betas_surv %>% select(15:16))

betas_new = drop_na(betas_surv %>% select(17:28))
surv_cancer_new = drop_na(betas_surv %>% select(29:30))
surv_noncancer_new = drop_na(betas_surv %>% select(31:32))
...
output$tbs_v1_base <- renderPlot({
 ...
 rect(xleft = 1064, xright = 2000, ybottom = -.1, ytop = 0.85, 
      border = NA, col = adjustcolor("red", alpha = 0.1))
 
 text(x = 1500, y = 0.75, # Coordinates
      label = "75% of liver offers", cex = 1, col= "red")
...
output$tbs_v1_predictions <- renderPlot({
  
  surv = tibble(tbs_out_current(), day = 1:1826)
  yr5 = surv %>% filter(day == 1826)
  
  yr5_without = round( yr5$m1_curve, 2)*100

  yr5_after = round(yr5$m2_curve, 2)*100

  par(mar = c(1, 0.5,0 , 0.5))
  plot(x=yr5_without, y=0.25, col ="#FF911C",bg ="#FF911C", pch= 25, cex = 3,
       xlim = c(0,100),
       ylim = c(0,0.45), 
       axes = FALSE
       #  xaxt='n',
  ...
  text(x = 50, y = 0.39, # Coordinates
       label = "Without transplant", cex = 1.5, col= "#FF911C")
  
  text(x = yr5_without, y = 0.31, # Coordinates
       label = paste0(yr5_without,"%"), cex = 2, col= "#FF911C")
  
  text(x = 50, y = 0.15, # Coordinates
       label = "After transplant", cex = 1.5, col= "#FF911C")
  
  text(x = yr5_after, y = 0.078, # Coordinates
       label = paste0(yr5_after,"%"), cex = 2, col= "#FF911C")
  
  points(x= yr5_after, y=0.022, col ="#FF911C",bg ="#FF911C", pch= 25, cex = 3)
  
  axis(side = 1, at=c(0, 25, 50, 75, 100), mgp=c(0.1,0.1,0.1), labels = c("0", "25%", "50%","75%","100%"), cex.axis = 1.5, tck = 1, lty = 2, col = "gray")
...
output$tbs_v2_predictions <- renderPlot({
  ...
  par(mar = c(1, 0.5,0 , 0.5))
  plot(x=yr5_without, y=0.25, col ="#0073B7",bg ="#0073B7", pch= 25, cex = 3,
       xlim = c(0,100),
       ylim = c(0,0.45), 
       axes = FALSE
       #  xaxt='n',
  ...
  text(x = 50, y = 0.39, # Coordinates
       label = "Without transplant", cex = 1.5, col= "#0073B7")
  
  text(x = yr5_without, y = 0.31, # Coordinates
       label = paste0(yr5_without,"%"), cex = 2, col= "#0073B7")
  
  text(x = 50, y = 0.15, # Coordinates
       label = "After transplant", cex = 1.5, col= "#0073B7")
  
  text(x = yr5_after, y = 0.078, # Coordinates
       label = paste0(yr5_after,"%"), cex = 2, col= "#0073B7")
  
  points(x= yr5_after, y=0.022, col ="#0073B7",bg ="#0073B7", pch= 25, cex = 3)
...
output$tbs_v2_base <- renderPlot({
 ...
 # rect(xleft = 1000, xright = 2000, ybottom = -.1, ytop = 0.85, 
  #     border = NA, col = adjustcolor("red", alpha = 0.1))
  
  text(x = 1500, y = 0.75, # Coordinates
       label = "Data not available", cex = 1, col= "red")
