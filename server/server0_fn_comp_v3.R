...
# Current TBS
fn_tbs_current <- function(centre_tbs,
													 rregistration_tbs,
													 rinpatient_tbs,
													 rwaiting_time_tbs,
													 rage_tbs,
	...
	rregistration_vec = group2dummy(as.numeric(rregistration_tbs), 8)
	...
	linear_prediction = dplyr::bind_cols(betas_current, x1, x2) %>% 
	...
	if(include_table){
		return(
			data.frame(
			  ...
			  #add survival curves
			  m1_curve = surv_active$m1_surv^exp(m1_linear_predictor), 
			  m2_curve = surv_active$m2_surv^exp(m2_linear_predictor)
		))
	} else {
		return(
			data.frame(m1_curve = surv_active$m1_surv^exp(m1_linear_predictor),
			           m2_curve = surv_active$m2_surv^exp(m2_linear_predictor)
...
# New TBS
fn_tbs_new <- function(centre_tbs,
											 rregistration_tbs,
											 rinpatient_tbs,
											 rwaiting_time_tbs,
											 rage_tbs,
	...
	# Make big prediction table
	linear_prediction = dplyr::bind_cols(betas_new, x1, x2) %>% 
		mutate(transformed_x1 = raw_x1^power,
					 transformed_x1 = ifelse(ln_1 == 1, log(transformed_x1), transformed_x1),
					 transformed_x2 = raw_x2^power,
					 transformed_x2 = ifelse(ln_2 == 1, log(transformed_x2), transformed_x2)
